...
| [044-cache](../features/044-cache/requirements.md) | Broker-local `IDistributedCache` reborn on RocksDB: an opt-in, **never-replicated** cache over a *separate* database (`dataDir/cache`), routed on the stock `hw:cache:*` raw-key surface (no new `HW.*`), TTL-bounded (default 24h / max 7d) and epoch-invalidated (cold after failover). Isolation proven structurally; `HybridCache` L2 smoke. Off by default | — | **Shipped** (2026-09-16) |
