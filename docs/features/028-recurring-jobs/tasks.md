...
### - [x] T0 — OD1–OD5 resolved with the user
...
### - [x] T1 — Schedule record, keys, and `HW.JOB SET|DEL|LIST`
...
### - [x] T2 — Fire-and-re-arm inside the promotion sweep
...
### - [x] T3 — Policies: missed fires and overlap
...
### - [x] T4 — The declaration API (per OD1) and expression validation
...
### - [x] T5 — Manifest and catalog visibility
...
### - [x] T6 — Dashboard: schedules visible; occurrences are ordinary messages
...
### - [x] T7 — Constraints, protocol invariants
...
- **Two Garnet API facts, found by probe, not assumed**: `SortedSetRange` with
  `limit: default` returns NOTHING (count 0), and `withScores: true` returns members without
  scores in the procedure API path. The second forced a framing improvement: the schedule
  record carries its own `nextFire` alongside the score.
- **A latent 025 defect fixed en route**: `Fail(...)` returns `false`, and returning it
  directly from `Prepare` aborts as a bare transient "Transaction failed" — inviting clients
  to retry permanently invalid input. HW.JOB hit it first (invalid expressions read as
  transient); the same slip existed in HW.SUBSCRIBE's 025 node-argument validation. Both now
  follow the convention (`Fail(...); return true;`) with a comment naming the trap.
- **OD4 changed at execution approval, not silently**: fire-anyway in v1; the
  skip-while-outstanding recommendation needs an ack-side completion hook, registered in the
  Deferred table with the prerequisite named.
- **The adversarial exactly-one-fire test bites**: with the re-arm's member removal
  deliberately broken, both it and the catch-up test fail (double fires); restored, 9/9 green.
- **`hw:job:index` is a const**, so `BoundedStructureTests`' helper-count guard doesn't see
  it; its bound is documented on the schedules row (precedent: `hw:reg:nodes`).
- **Verified live**: one fire per interval over 75 seconds in the samples, manifest line at
  boot, `/api/jobs` showing next/last fire and hosted state.
